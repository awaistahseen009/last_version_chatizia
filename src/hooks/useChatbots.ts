...
export const useChatbots = () => {
  ...
  const { user } = useAuth();
  const { isWithinLimits, refetch: refetchLimits } = useSubscriptionLimits();
  ...
  const createChatbot = async (chatbotData: Omit<Chatbot, 'id' | 'user_id' | 'created_at' | 'updated_at' | 'conversations_count' | 'messages_this_month' | 'response_time'>) => {
    ...
    if (!isWithinLimits('chatbots')) {
      throw new Error('Subscription limit reached for chatbots');
    }
    ...
    try {
      ...
      addNotification({
        ...
        message: `Your chatbot "${newBot.name}" has been created successfully.`,
        type: 'chatbot'
      });
  ...
  const updateChatbot = async (id: string, updates: Partial<Chatbot>) => {
    try {
      ...
      addNotification({
        ...
        message: `Your chatbot "${data.name}" has been updated successfully.`,
        type: 'chatbot'
      });
  ...
  const deleteChatbot = async (id: string) => {
    try {
      // Get chatbot name before deleting
      ...
      addNotification({
        ...
        message: `Your chatbot "${chatbotName}" has been deleted.`,
        type: 'chatbot'
      });
    } catch (err) {
