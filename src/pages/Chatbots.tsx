...
const Chatbots: React.FC = () => {
  ...
  const [selectedBotForExport, setSelectedBotForExport] = useState<string | null>(null);
  ...
  const handleExportBot = (botId: string) => {
    setSelectedBotForExport(botId);
    ...
    <div className="space-y-6">
      ...
      {showExportImport && (
        <div className="bg-white rounded-lg border border-slate-200 p-6">
          ...
          <ExportImportChatbot 
            chatbotId={selectedBotForExport || undefined} 
            onSuccess={() => {
              setShowExportImport(false);
              setSelectedBotForExport(null);
            }}
          />
        </div>
      )}
                    ...
                    {showDropdown === bot.id && (
                      <div className="absolute right-0 top-8 w-48 bg-white rounded-lg shadow-lg border border-slate-200 z-10">
                        <div className="py-1">
                          ...
                          <button
                            onClick={() => handleExportBot(bot.id)}
                            className="w-full flex items-center space-x-2 px-4 py-2 text-left text-slate-700 hover:bg-slate-50"
                          >
                            <Download className="w-4 h-4" />
